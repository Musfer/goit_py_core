...
def sort_files(dirname, root_path=root_folder_path):  # rootdir is True is we are in root_path falder
...
def main():
    try:  # check if folder  exists
        ...
        sys.exit()
        #       uncomment to test on Trashfolder
        # try:
        #     shutil.rmtree(r"\python_projects\GoIT\goit_task6\TrashfolderCopy")
        # finally:
        #     shutil.copytree(r"\python_projects\GoIT\goit_task6\Trashfolder",
        #                 r"\python_projects\GoIT\goit_task6\TrashfolderCopy")  # for test purposes
        #     root_folder_path = Path(r"\python_projects\GoIT\goit_task6\TrashfolderCopy")
    ...
    #      uncomment to print name all of found files and folders with
    # print(f"List of renamed files:", file=logs)
    # for name in files_renamed:
    #     try:
    #         print(f"\t{name}", file=logs)
    #     except Exception as err:
    #         pass
    #         print(err)
    #         print(name)

    with open(root_folder_path.joinpath("logs.txt"), "w", encoding="utf-8") as logs:
